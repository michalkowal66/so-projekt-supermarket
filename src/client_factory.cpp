...
void handle_sigint(int signo) {
    running = false;
    std::cout << "\nClient Factory: Received SIGINT. Stopping client creation." << std::endl;
}
...
void* wait_children(void *arg) {
    while (running) {
        ...
        if (pid > 0) {
            children--;
            std::cout << "Client Factory: Process (PID: " << pid << ") finished." << std::endl;
        } else {
        }
    }
    ...
    while (true) {
        ...
        if (pid > 0) {
            children--;
            std::cout << "Client Factory: Process (PID: " << pid << ") finished after shutdown." << std::endl;
        } else {
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 2) {
        std::cout << "Client Factory: Max children count not provided, using default value: " << max_children << std::endl << std::endl;
    }
    else {
        try {
            max_children = std::stoi(argv[1]);
        } catch (const std::exception& e) {
            std::cout << "Client Factory: Max children count parsing error, using default value: " << max_children << std::endl << std::endl;
        }

        if (max_children > MAX_CHILDREN_DEF) {
            max_children = MAX_CHILDREN_DEF;
            std::cout << "Client Factory: Max children count can not exceed max value: " << MAX_CHILDREN_DEF << ", using max value." << std::endl << std::endl;
        }
        else if (max_children < MIN_CHILDREN_DEF) {
            max_children = MIN_CHILDREN_DEF;
            std::cout << "Client Factory: Max children count can not be negative, using minimum value: " << MIN_CHILDREN_DEF << std::endl << std::endl;
        }
    }
    ...
    std::cout << "Client Factory (" << getpid() << "): Running..." << std::endl;
    ...
    while (running) {
        ...
        if (pid == 0) {
            ...
            children++;
            std::cout << "Client Factory: Created client process (PID: " << pid << ")." << std::endl;
        } else {
            perror("fork");
        }
    ...
    std::cout << "Client Factory: Stopping client creation. Waiting for active clients to finish." << std::endl;

    // Dołączanie wątku odpowiedzialnego za czyszczenie procesów potomnych
    pthread_join(wait_children_thread, NULL);

    std::cout << "Client Factory: All clients have finished. Exiting." << std::endl;
    return 0;
}
